...
def convertToMessage(data,attributeName):
    ...
    for dt in data:
        ...
        index += 1
    return response
class ConversationUser(ModelViewSet):
...
class ChatBotAPIView(APIView):
    def post(self, request):
        ...
        newAnswer = None
        if conversationFound.lastCommand == 'EXIT':
            answer.command == None
        ...
        if answer.command == 'SEARCH_BOOKING':
            booking = Booking.objects.all()
            finalMessage += convertToMessage(booking,'comment')
            newAnswer = Conversation(type="A",message =finalMessage if answer.additionalMessage is None else finalMessage + '\n'+ answer.additionalMessage, history= conversationFound)
        
        if answer.command == 'SEARCH_PAYMENTS':
            payments = Payment.objects.all()
            finalMessage += convertToMessage(payments,'status')
            newAnswer = Conversation(type="A",message =finalMessage if answer.additionalMessage is None else finalMessage + '\n'+ answer.additionalMessage, history= conversationFound)
